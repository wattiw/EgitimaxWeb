...
class QuestionOptionsWithSolution extends StatelessWidget {
  ...
  TblQueQuestionMain? question;
  List<TblQueQuestionOption>? options;
  List<TblQueQuestionAchvMap>? achievementMap;
  AppRepository appRepository = AppRepository();

  QuestionOptionsWithSolution({Key? key, required this.questionId})
      : super(key: key);

  Future<void> loadData() async {
    question = await appRepository.getTblQueQuestionMain(questionId);
    options =
  ...
  Widget build(BuildContext context) {
    if (questionId <= BigInt.zero || questionId == null) {
      return Container();
    }

    return FutureBuilder<void>(
      future: loadData(),
      builder: (context, snapshotLoadData) {

        if (snapshotLoadData.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        } else if (snapshotLoadData.hasError) {
          return Center(
              child: Text(
            "Error: ${snapshotLoadData.error}",
          ...
          var questionDetails = [
            FutureBuilder<TblUtilAcademicYear?>(
              future: appRepository
                  .getTblUtilAcademicYear(question!.academicYear ?? 0),
            ...
            FutureBuilder<TblUtilDifficulty?>(
              future: appRepository
                  .getTblUtilDifficulty(question!.difficultyLev ?? 0),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator());
                } else if (snapshot.hasError) {
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return FutureBuilder<TblUtilBranch?>(
                    future: appRepository
                        .getTblUtilBranch(snapshot.data?.branchId ?? 0),
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        return const Center(child: CircularProgressIndicator());
                      } else if (snapshot.hasError) {
